...
def beat_effect_list(effect, roundTempo):
    effects = []
    ...
    if(effect.mixTableChange and effect.mixTableChange.tempo):
        ...
        if roundTempo:
            ...
            effects.append("bfx:tempo_change:%s"% effect.mixTableChange.tempo.value)
...
def guitarpro2tokens(song, artist, roundTempo=True, verbose=False):
    ...
    if roundTempo:
        ...
        tempo_token = "tempo:%s" % song.tempo
        ...
        for t, track in enumerate(song.tracks):
            ...
            for v, voice in enumerate(measure.voices):
                ...
                for b, beat in enumerate(voice.beats):
                    ...
                    beatfx = beat_effect_list(beat.effect, roundTempo)
                    if(len(beatfx)):
                        event = {"type": "beatfx", 
                                 "effects": beatfx,
                                 "duration": beat_duration,
...
file_path = files(__package__).joinpath("data/blank.gp5")
blankgp5 = gp.parse(file_path)
blankgp5.tracks = []
...
def tokens2guitarpro(all_tokens, verbose=False):
    ...
    blankgp5 = gp.parse(files(__package__).joinpath("data/blank.gp5"))
    blankgp5.tracks = []
...
# guitar pro --> tokens
def dadagp_encode(input_file, output_file, artist_token, roundTempo=True):
    song = gp.parse(input_file)
    # Convert the song to tokens
    tokens = guitarpro2tokens(song, artist_token, roundTempo, verbose=False)
    # Write the tokens to text file
    f = open(output_file, "w")
    f.write("\n".join(tokens))
    f.close()

def dadagp_batch_encode(input_dir, output_dir, artist_token, roundTempo=True):
    count = 0
    bad_files = []
    for root, dirs, files in os.walk(input_dir):
        for file in tqdm(files):
            file_path = os.path.join(root, file)
            output_name = os.path.join(output_dir, file.split('.gp5')[0] + '.txt')
            try:
                dadagp_encode(file_path,output_name, artist_token, roundTempo)
            except: 
                bad_files.append(file_path)

    print('{} files messed up in encoding'.format(len(bad_files)))
...
def dadagp_batch_decode(input_dir, output_dir):
    ...
    count = 0
    for f in files:
        output_name = os.path.join(output_dir, f.split('_id_')[1].split('.')[0] + '.' + 'gp5')
        print(f)
...
def dadagp_decode(input_file, output_file):
    ...
    try:
        ...
        print("Error in tokens2guitarpro:", e)
